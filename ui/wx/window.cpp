...
public:
  ToolPanel(wxWindow* parent) : wxPanel(parent, wxID_ANY) {
	...
	picker->Bind(
	    wxEVT_COLOURPICKER_CHANGED, [this](wxColourPickerEvent&) -> void {
  ...
  void Render(wxDC& dc) {
	...
	for (int i = 0; i < GRID_SIZE; i++) {
	  for (int j = 0; j < GRID_SIZE; j++) {
		...
		dc.DrawRectangle((i * cellWidth) + 2, (j * cellHeight) + 2,
...
MainWindow::MainWindow()
    : wxFrame(NULL, -1, "(new file) -- Kovel - Voxel Editor", wxDefaultPosition,
  ...
  // Packaging (Debian, DESKTOP file, Windows MSI, Reg for MIME TYPE and
  // actions)
  ...
  Bind(wxEVT_MENU, [core, this](wxCommandEvent&) -> void {
	...
	if (metadata->ShowModal() == wxID_CANCEL)
	  return;
  ...
  Bind(wxEVT_MENU, [this](wxCommandEvent&) -> void {
	wxFileDialog* fileDlg = new wxFileDialog(this, "Open Kovel file", "", "",
	                                         "Kovel files (*.kvl)|*.kvl",
  ...
  Bind(wxEVT_MENU, [core, this](wxCommandEvent&) -> void {
	if (this->GetLabel().StartsWith("(new file)")) {
	  wxFileDialog* fileDlg = new wxFileDialog(
	      this, "Save Kovel file", "", "", "Kovel file (*.kvl)|*.kvl",
  ...
  Bind(wxEVT_MENU, [core, this](wxCommandEvent&) -> void {
	wxFileDialog* fileDlg = new wxFileDialog(this, "Save Kovel file", "", "",
	                                         "Kovel file (*.kvl)|*.kvl",
	                                         wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
	if (fileDlg->ShowModal() == wxID_CANCEL)
	  return;
  ...
  Bind(wxEVT_MENU, [core, this](wxCommandEvent&) -> void {
	wxFileDialog* fileDlg = new wxFileDialog(this, "Save JSON file", "", "",
	                                         "JSON file (*.json)|*.json",
	                                         wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
	if (fileDlg->ShowModal() == wxID_CANCEL)
	  return;
	...
	if (!ret)
	  wxMessageBox("Please, save your file first", "Error",
	               wxOK | wxICON_ERROR);
  }, 63);

  file->Append(64, "Export as Collada DAE...");
  Bind(wxEVT_MENU, [core, this](wxCommandEvent&) -> void {
	wxFileDialog* fileDlg = new wxFileDialog(this, "Save Collada DAE file", "",
	                                         "", "Collada DAE (*.dae)|*.dae",
	                                         wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
	if (fileDlg->ShowModal() == wxID_CANCEL)
	  return;
  ...
  file->Append(exit);
  Bind(wxEVT_MENU, [this](wxCommandEvent&) -> void { this->Close(); },
       wxID_EXIT);
  ...
  Bind(wxEVT_MENU,
       [preview](wxCommandEvent&) -> void { preview->zoom += 1.0f; }, 68);
  navigate->Append(69, "Zoom &out");
  Bind(wxEVT_MENU,
       [preview](wxCommandEvent&) -> void { preview->zoom -= 1.0f; }, 69);
  navigate->AppendSeparator();
  navigate->Append(70, "Rotate left");
  Bind(wxEVT_MENU,
       [preview](wxCommandEvent&) -> void { preview->rotation += 15; }, 70);
  navigate->Append(71, "Rotate right");
  Bind(wxEVT_MENU,
       [preview](wxCommandEvent&) -> void { preview->rotation -= 15; }, 71);
  menuBar->Append(navigate, "&Navigate");

  wxMenu* help = new wxMenu;
  help->Append(wxID_ANY,
               "Legacy OpenGL 1.0"); // Change to GL ES 2.0 in a future
  ...
  Bind(wxEVT_MENU, [this](wxCommandEvent&) -> void {
	...
	aboutInfo.AddDeveloper(
	    wxString::FromUTF8(std::string("Adrián Arroyo Calle").c_str()));
