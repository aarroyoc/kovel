...
public:
	...
	{
		gl=new GlPreviewer();
		canvas=new BGLView(Bounds(),"GL",B_FOLLOW_ALL_SIDES,0,BGL_RGB | BGL_ALPHA | BGL_DOUBLE);
		AddChild(canvas);
	}
...
PreviewPanel::PreviewPanel() : BView(NULL,B_WILL_DRAW | B_PULSE_NEEDED)
{
	...
	BColorControl* color=new BColorControl(BPoint(0,0),B_CELLS_32x8,5.0,NULL,new BMessage(6));
