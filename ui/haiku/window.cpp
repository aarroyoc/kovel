...
const uint32 NEW_FILE = 0, OPEN_FILE = 1, SAVE_FILE = 2, SAVE_FILEAS = 3,
             UNDO = 4, CLEARGRID = 5, SELECT_COLOR = 6, UP = 7, DOWN = 8,
             RLEFT = 9, RRIGHT = 10;
const uint32 ZOOM_IN = 11, ZOOM_OUT = 12, META = 13, READ_FILE = 14,
             SAVE_FILE_DO = 15;
const uint32 EXPORT_DAE = 16, EXPORT_DAE_DO = 17, EXPORT_JSON = 18,
             EXPORT_JSON_DO = 19;
  ...
  void Draw(BRect rect) {
	...
	for (int i = 0; i < GRID_SIZE; i++) {
	  for (int j = 0; j < GRID_SIZE; j++) {
		...
		this->StrokeRect(BRect(i * cellWidth, j * cellHeight,
		...
		this->FillRect(BRect((i * cellWidth) + 4, (j * cellHeight) + 4,
		                     (i * cellWidth) + 4 + cellWidth - 8,
		                     (j * cellHeight) + 4 + cellHeight - 8));
	  }
	}
  }
...
KovelWindow::KovelWindow()
    : BWindow(BRect(30, 30, 330, 530), "(new file) -- Kovel - Voxel Editor",
              B_TITLED_WINDOW, 0) {
  Core* core = Core::Instance();
  ...
  BView* main = new BView(NULL, B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE |
                                    B_FRAME_EVENTS | B_DRAW_ON_CHILDREN);
  main->SetViewColor(220, 220, 220);
  main->SetLayout(mainSizer);
  tool = new ToolPanel();

  BView* work = new BView(NULL, B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE |
                                    B_FRAME_EVENTS | B_DRAW_ON_CHILDREN);
...
BMenuBar* KovelWindow::CreateMenuBar() {
  /* MenuBar */
  ...
  BMenuItem* exportDAE =
      new BMenuItem("Export as DAE...", new BMessage(EXPORT_DAE));
  file->AddItem(exportDAE);

  BMenuItem* exportJSON =
      new BMenuItem("Export as JSON...", new BMessage(EXPORT_JSON));
  ...
  BMenuItem* clear =
      new BMenuItem("Clear current grid", new BMessage(CLEARGRID));
  edit->AddItem(clear);
  /*BMenuItem* select=new BMenuItem("Select colour",new BMessage(SELECT_COLOR));
  edit->AddItem(select);*/

  BMenuItem* about =
      new BMenuItem("About Kovel", new BMessage(B_ABOUT_REQUESTED));
...
void KovelWindow::LoadFile(BString path) {
  Core* core = Core::Instance();
  FILE_PATH = path;
...
bool KovelWindow::QuitRequested() {
  be_app_messenger.SendMessage(B_QUIT_REQUESTED);
  return BWindow::QuitRequested();
}

void KovelWindow::MessageReceived(BMessage* msg) {
  Core* core = Core::Instance();
  switch (msg->what) {
  case NEW_FILE: {
  ...
  case OPEN_FILE: {
	BFilePanel* filep =
	    new BFilePanel(B_OPEN_PANEL, new BMessenger(this), NULL, B_FILE_NODE,
	                   false, new BMessage(READ_FILE));
	filep->Show();
	break;
  }
  case SAVE_FILE: {
	...
	BFilePanel* filep = new BFilePanel(B_SAVE_PANEL, new BMessenger(this), NULL,
	                                   B_FILE_NODE, false, mesg);
	filep->Show();
	break;
  }
  case EXPORT_DAE: {
